import { useEffect, useId, useState, useRef, useMemo, type ChangeEvent, type DragEvent } from "react";
import type { DevelopmentImage, NewDevelopmentImage } from "@/interfaces/development.interface";
import {
  MAX_PROPERTY_IMAGES,
...
type UnifiedItem =
  | { kind: "existing"; origIdx: number; img: DevelopmentImage }
  | { kind: "new"; origIdx: number; img: NewDevelopmentImage };
...
type ImageGridUploaderProps = {
  images: NewDevelopmentImage[];
  existingImages?: DevelopmentImage[];
  ...
  onReorderImages?: (existingImages: DevelopmentImage[], newImages: NewDevelopmentImage[]) => void;
  label?: string;
  error?: string;
};
...
export function ImageGridUploader({
  ...
  const inputId = `${uploaderId}-images-input`;
  const radioGroupName = `${uploaderId}-primary-image`;
        ...
        >
          <div className="flex flex-col items-center justify-center pb-6 pt-5 text-slate-500">
            ...
            <p className="mt-1 text-[11px] text-slate-400">
              ...
              {MAX_PROPERTY_IMAGES_TOTAL_SIZE_MB} MB por envío.
            </p>
          </div>
          <input
            id={inputId}
